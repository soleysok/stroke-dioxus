...
//! `localStorage`, and which character the handwriting pad opened in
//! `sessionStorage`.
//!
//! There is no account system yet, so the two lists are the whole of "My Lists"
//! for now. Values are stored as a plain string of characters rather than JSON,
//! which keeps them small and readable in devtools.
